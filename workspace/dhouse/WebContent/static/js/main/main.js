...
const banners = document.querySelector("div.banners"); // 배너들을 감싼 총 div
const imageDiv = document.querySelectorAll("div.banners div.banner"); // 각각의 이미지가 담기는 배너 리스트
...
const next = document.querySelector("div.next"); // 다음 버튼
const prev = document.querySelector("div.prev"); // 이전 버튼
const buttons = document.querySelectorAll(".buttons button"); // 버튼 2개 모두의 리스트
const $bannerActive = $(".banner-active"); // 움직이는 배너 바
const bannerPercentage = (100 / (imageDiv.length - 2)); // 배너 바 매번 증가해야하는 수치
// const lastImageDiv = document.querySelector("div.banners div.banner[0]");
// const firstImageDiv = document.querySelector("div.banners div.banner[parseInt(imageDiv.length)]");
...
banners.style.transform = `translate(-1519px)`;
...
function autoSlide(){
    ...
    if(count == 7) { /* 6->7로 넘어가게 되면*/
        ...
        setTimeout(function(){ 
            ...
            banners.style.transform = `translate(-1519px)`; /* 이동 하는 배너 위치 */
        }, 300);
    }
...
prev.addEventListener("click", function(){
    ...
    if(count == 0) {
        ...
        setTimeout(function(){
            ...
            banners.style.transform = `translate(${-1519 * (imageDiv.length)}px)`;
        }, 300);
    }
    changeButtonStyle();
    auto = setInterval(autoSlide, 2000);
